  ...
  useEffect(() => {
    const io = new IntersectionObserver(([e]) => {
      ...
      const tick = () => {
        ...
        if (start < frames) { rafRef.current = requestAnimationFrame(tick); }
        else { setV(to); rafRef.current = null; }
      };
  ...
  return (
    <form className="contact-form" onSubmit={submit} noValidate>
      <div className="cf-row">
        <div className="cf-group"><label>Your Name</label><input value={vals.name} onChange={set("name")} placeholder="Jane Doe" required /></div>
        <div className="cf-group"><label>Email Address</label><input type="email" value={vals.email} onChange={set("email")} placeholder="jane@email.com" required /></div>
      </div>
...
export default function Home() {
  ...
  // Check if user is already logged in so we can show "Dashboard" instead of "Sign In"
    ...
    <div className="hp">
      ...
      <section id="hero" className="hero">
        ...
        <div className="hero-content">
          <div className="hero-chip"><span className="chip-dot" />Live Forecast Engine</div>
          <h1 className="hero-h1">Know exactly<br /><em>how much sun</em><br />tomorrow holds.</h1>
          <p className="hero-sub">
            Feed in your panel specs, drop a pin on the map. PVForecast pulls real
            meteorological data and returns a precise 48-hour production curve — updated every time.
          </p>
          <div className="hero-btns">
            {isLoggedIn ? (
              <Link to="/User" className="btn-glow">Go to Dashboard →</Link>
            ) : (
              <>
      ...
      <section className="metrics reveal">
        <div className="metric"><div className="metric-num"><Count to={99} suffix="%" />.<Count to={2} /></div><div className="metric-lbl">Forecast accuracy</div></div>
        <div className="metric-sep" />
        <div className="metric"><div className="metric-num"><Count to={48} suffix="h" /></div><div className="metric-lbl">Prediction window</div></div>
        <div className="metric-sep" />
        <div className="metric"><div className="metric-num"><Count to={15} suffix="m" /></div><div className="metric-lbl">Data granularity</div></div>
        <div className="metric-sep" />
        <div className="metric"><div className="metric-num">∞</div><div className="metric-lbl">Panel configurations</div></div>
      </section>
        ...
        <div className="feat-grid">
          {[
            ...
            <div className="feat-card reveal" key={i} style={{ animationDelay: `${i * 0.08}s` }}>
              <div className="feat-top"><span className="feat-icon">{f.icon}</span><span className="feat-num">{f.num}</span></div>
      ...
      <section id="about" className="about-section">
        <div className="about-inner">
          <div className="about-left reveal">
            ...
            <p>PVForecast started as a personal project to solve a simple frustration: existing solar monitoring apps were either too expensive, too complex, or simply didn't connect panel specs to real forecast data.</p>
            <p>We built the tool we wished existed — one that takes your exact installation geometry, queries live meteorological data, and returns an honest production estimate you can actually rely on.</p>
            <p>Whether you're a homeowner with 4 panels or an engineer managing a commercial array, PVForecast gives you the precision insight to optimise self-consumption, plan battery charging, and stop guessing.</p>
            <div className="about-team">
              <div className="team-head">The team</div>
              <div className="team-card reveal">
                <div className="tc-avatar">YB</div>
                <div><strong>Yassine Bk</strong><span>Full-Stack Engineer · Built the entire forecast pipeline, React dashboard, and backend API from scratch.</span></div>
              </div>
            </div>
          </div>
          <div className="about-right reveal">
            <div className="about-card">
              ...
              {[["Forecast horizon","48 hours"],["Resolution","15-minute intervals"],["Panel types","Any PV configuration"],["Data source","forecast.solar API"],["Authentication","bcrypt password hashing"],["Map engine","Leaflet + OpenStreetMap"],["Charts","Recharts / SVG"],["Coverage","Global 🌍"]].map(([k,v]) => (
                <div className="ac-row" key={k}><span>{k}</span><strong>{v}</strong></div>
              ))}
            </div>
      ...
      <section id="contact" className="contact-section">
        <div className="contact-inner">
          <div className="contact-left reveal">
            ...
            <p>Questions about your setup? Feature request? Bug report? Reach out and we'll get back to you within a day.</p>
            <div className="contact-links">
              <div className="cl-item"><div className="cl-icon">✉</div><div><strong>Email</strong><span>hello@pvforecast.app</span></div></div>
              <div className="cl-item"><div className="cl-icon">◎</div><div><strong>GitHub</strong><span>github.com/pvforecast</span></div></div>
              <div className="cl-item"><div className="cl-icon">⚡</div><div><strong>Response time</strong><span>Usually within 24 hours</span></div></div>
            </div>
          </div>
          <div className="contact-right reveal"><ContactForm /></div>
        </div>
      </section>
      ...
      <footer className="footer">
        <div className="footer-top">
          <div className="footer-brand">
            ...
            <div><div className="fb-name">PVForecast</div><div className="fb-tag">Solar production intelligence</div></div>
          </div>
          <div className="footer-links">
            <div className="fl-col"><strong>Product</strong><button onClick={() => scrollTo("features")}>Features</button><Link to="/Signup">Sign up free</Link></div>
            <div className="fl-col"><strong>Company</strong><button onClick={() => scrollTo("about")}>About us</button><button onClick={() => scrollTo("contact")}>Contact</button></div>
            <div className="fl-col"><strong>Account</strong>
              {isLoggedIn
                ? <Link to="/User">Dashboard</Link>
                : <><Link to="/Login">Sign in</Link><Link to="/Signup">Register</Link></>
              }
            </div>
          </div>
        </div>
        <div className="footer-bottom">
          ...
          <button className="footer-theme" onClick={toggleTheme}>{theme === "dark" ? "☀️ Light mode" : "🌙 Dark mode"}</button>
        </div>
      </footer>
